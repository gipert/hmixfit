...
HMixFit::HMixFit(json outconfig) : config(outconfig) {
  ...
  for (json::iterator el = config["fit"]["parameters"].begin();
       el != config["fit"]["parameters"].end(); ++el) {


    auto tmp_use_priors = el.value().value("use-prior", true);
    ...
    if (already_exists) {
      ...
      if (el.value().contains("range")) {
        ...
        if (!el.value()["fixed"].is_number()) {
          throw std::runtime_error("\"fixed\" value of parameter " + el.key() +
                                   " is not a number");
        }
        this->GetParameters().Add(el.key(),
    ...
    if (el.value().contains("prior") && (tmp_use_priors==true)) {
      ...
      if (prior_cfg.contains("TFormula") and prior_cfg.contains("histogram")) {
        throw std::runtime_error("please choose either \"TFormula\" or "
                                 "\"histogram\" for parameter " +
                                 el.key());
      }
      ...
      if (prior_cfg.contains("histogram")) {
        ...
        if (expr.find(':') == std::string::npos) {
          throw std::runtime_error(
              "invalid \"histogram\" format for parameter " + el.key());
        }
        auto filename = expr.substr(0, expr.find_first_of(':'));
        auto objname =
            expr.substr(expr.find_first_of(':') + 1, std::string::npos);
        TFile _tff(filename.c_str());
        if (!_tff.IsOpen())
          throw std::runtime_error("invalid ROOT file: " + filename);
        auto obj = _tff.Get(objname.c_str());
        if (!obj)
          throw std::runtime_error("could not find object '" + objname +
                                   "' in file " + filename);
        if (obj->InheritsFrom(TH1::Class())) {
          prior_hist = dynamic_cast<TH1 *>(obj);
          // the following is a workaround for BAT's bad implementation of the
          // BCTH1Prior constructor
          prior_hist->SetDirectory(nullptr);
          _tff.Close();
        } else
          throw std::runtime_error("object '" + objname + "' in file " +
                                   filename + " is not an histogram");
        BCLog::OutDebug("found histogram prior '" + expr + "' for parameter '" +
                        el.key() + "'");
      }
      // is a tformula
        ...
        TF1 _tformula = utils::ParseTFormula(
            el.key(), expr, el.value()["range"][0].get<double>(),
            el.value()["range"][1].get<double>());
        BCLog::OutDebug("found TFormula prior '" + expr + "' for parameter '" +
                        el.key() + "'");

        // I did not manage to use BCTF1Prior from BAT, it segfaults for
        // misterious reasons so I just make a temporary histogram out of the
        // TFormula
        prior_hist = new TH1D((el.key() + "_prior_h").c_str(), "h1_prior", 1000,
                              el.value()["range"][0].get<double>(),
                              el.value()["range"][1].get<double>());
        for (int j = 1; j <= prior_hist->GetNbinsX(); j++) {
          prior_hist->SetBinContent(
              j, _tformula.Eval(prior_hist->GetBinCenter(j)));
        }
        BCLog::OutDebug(
            "produced temporary prior 1000-bin histogram for parameter '" +
            el.key() + "'");
      } else
        throw std::runtime_error(
            "supported prior types: 'histogram', 'TFormula'");

      prior = new BCTH1Prior(prior_hist);
      delete prior_hist;
      BCLog::OutDetail("assigned prior '" + expr + "' to parameter '" +
                       el.key() + "'");

      if (prior != nullptr) {
        if (!prior->IsValid()) {
          throw std::runtime_error("invalid prior set for parameter " +
                                   el.key());
        }
  ...
  for (auto &el : config["fit"]["theoretical-expectations"].items()) {
    ...
    if (!_tf.IsOpen())
      throw std::runtime_error("invalid ROOT file: " + el.key());

    // loop over requested histograms in file
    for (auto &elh : el.value().items()) {

      livetime = elh.value()["livetime"].get<double>();
      mc_path = elh.value().value("pdf-path", "");
      ...
      if (elh.value().contains("rebin-factor") and
          elh.value()["rebin-factor"].is_string()) {
        BCLog::OutDetail("using specified variable-sized rebin for dataset '" +
                         el.key() + "'");
        change_points = utils::ParseBinChangePoints(
            elh.value()["rebin-factor"].get<std::string>());
        variable_bin = true;
      } else if (elh.value().contains("rebin-factor") and
                 elh.value()["rebin-factor-x"].is_string()) {
        change_points_x = utils::ParseBinChangePoints(
            elh.value()["rebin-factor-x"].get<std::string>());
        variable_bin = true;

      } else if (elh.value().contains("rebin-factor") and
                 elh.value()["rebin-factor-y"].is_string()) {
        change_points_y = utils::ParseBinChangePoints(
            elh.value()["rebin-factor-y"].get<std::string>());
        variable_bin = true;

      } else {
        rebin_x = elh.value().value("rebin-factor-x", 1) *
                  elh.value().value("rebin-factor", 1);
        rebin_y = elh.value().value("rebin-factor-y", 1);

        if (rebin_x != 1)
          BCLog::OutDetail("using rebin X factor = " + std::to_string(rebin_x) +
                           " for dataset '" + elh.key() + "'");
        if (rebin_y != 1)
          BCLog::OutDetail("using rebin Y factor = " + std::to_string(rebin_y) +
                           " for dataset '" + elh.key() + "'");
      }

      BCLog::OutDebug("getting data histogram '" + elh.key() + "' from " +
                      el.key());
      auto th_orig = dynamic_cast<TH1 *>(_tf.Get(elh.key().c_str()));
      if (!th_orig)
        throw std::runtime_error("could not find object '" + elh.key() +
                                 "' in file " + el.key());

      // set explicative histogram names
      auto basename = el.key().substr(el.key().find_last_of('/') + 1,
                                      el.key().find_last_of('.') -
      ...
      if (variable_bin == false) {
        th = dynamic_cast<TH1 *>(
            th_orig->Clone(utils::SafeROOTName(basename).c_str()));
        th->Rebin(rebin_x);
        ...
        if (th_orig->GetDimension() == 1) {
          th = dynamic_cast<TH1 *>(th_orig->Rebin(
              change_points.size() - 1, utils::SafeROOTName(basename).c_str(),
              &change_points[0]));
          ...
          th2 = new TH2D(utils::SafeROOTName(basename).c_str(),
                         ...
                         change_points_x.size() - 1, &change_points_x[0],
                         change_points_y.size() - 1, &change_points_y[0]);

          TAxis *xaxis = th_orig->GetXaxis();
          TAxis *yaxis = th_orig->GetYaxis();
          for (int j = 1; j <= yaxis->GetNbins(); j++) {
            for (int i = 1; i <= xaxis->GetNbins(); i++) {
              th2->Fill(xaxis->GetBinCenter(i), yaxis->GetBinCenter(j),
                        th_orig->GetBinContent(i, j));
            }
          }
          th = dynamic_cast<TH2 *>(th2);
        } else {
          throw std::runtime_error(
              "Variable binning cannot be applied (yet?) to TH3");
        }
      }
      _current_ds.data = th;
      ...
      for (auto &it : elh.value()["components"]) {
        ...
        auto sum_parts = [&](std::string i) {
          ...
          if (it["part"].is_object()) {
            for (auto &p : it["part"].items()) {
              ...
              if (path_to_part.back() == '/')
                path_to_part.pop_back();
              auto part =
                  path_to_part.substr(path_to_part.find_last_of('/') + 1);
              path_to_part.erase(path_to_part.find_last_of('/'));
              auto volume =
                  path_to_part.substr(path_to_part.find_last_of('/') + 1);
              auto filename = prefix + "/" + p.key() + "/" + true_iso + "/" +
                              "pdf-" + volume + "-" + part + "-" + i + ".root";
              BCLog::OutDebug("opening file " + filename);
              BCLog::OutDebug("summing object '" + elh.key() + " with weight " +
                              std::to_string(p.value().get<double>()));
              // get histogram
              int nprim_tmp;
              auto thh = this->GetFitComponent(filename, elh.key(), nprim_tmp,
                                               _current_ds.data_orig);
              // add it with weight
              if (!sum) {
                sum = thh;
            ...
            auto volume =
                path_to_part.substr(path_to_part.find_last_of('/') + 1);
            auto filename = prefix + "/" + it["part"].get<std::string>() + "/" +
                            true_iso + "/" + "pdf-" + volume + "-" + part +
                            "-" + i + ".root";
            BCLog::OutDebug("getting object '" + elh.key() + "' in file " +
                            filename);
            // get histogram
            int nprim = 0;
            auto thh = this->GetFitComponent(filename, elh.key(), nprim,
                                             _current_ds.data_orig);

            return thh;
          } else
            throw std::runtime_error(
                "unexpected 'part' value found in [\"fit\"][\"" + el.key() +
                "\"][\"" + elh.key() + "\"]");
        };
        /* END INTERMEZZO */
        ...
        for (auto &iso : it["components"].items()) {
          BCLog::OutDetail("adding a PDF for the '" + elh.key() +
                           "' dataset with '" + iso.key() +
                           "' as scaling parameter (debug mode for details)");
          ...
          if (!exists)
            throw std::runtime_error(
                "fit parameter '" + iso.key() +
                "' not found, is it defined in \"parameters\"?");
          ...
          if (!iso.value().contains("TFormula") and it.contains("root-file")) {
            ...
            auto weight = iso.value().value("weight",1.);
            std::cout<<"Weight for "<<iso.key()<<" "<<weight<<std::endl;
            int nprim;
            auto thh = this->GetFitComponent(
                mc_path + it["root-file"].get<std::string>(), obj_name, nprim,
                _current_ds.data_orig, weight);
            ...
            _current_ds.comp.insert(
                {comp_idx, utils::ReformatHistogram(thh, _current_ds.data)});
          }
          // it's a user defined file list with weights
          else if (!iso.value().contains("TFormula") and
                   it.contains("root-files")) {
            BCLog::OutDebug(
                "user-specified ROOT file list (with weights) detected");
            auto obj_name = iso.value().value("hist-name", elh.key());
            ...
            if (it["root-files"].is_object()) {
              for (auto &p : it["root-files"].items()) {
                ...
                BCLog::OutDebug("summing object '" + obj_name +
                                " with weight " +
                                std::to_string(p.value().get<double>()));
                int nprim_tmp;
                auto thh = this->GetFitComponent(p.key(), obj_name, nprim_tmp,
                                                 _current_ds.data_orig);
                // add it with weight
                if (!sum) {
                  sum = thh;
              ...
              throw std::runtime_error("\"root-files\" must be a dictionary");
            sum->SetName(utils::SafeROOTName(iso.key()).c_str());
            _current_ds.comp_orig.insert({comp_idx, sum});
            _current_ds.number_simulated.insert(
                {comp_idx, std::numeric_limits<double>::max()});
            _current_ds.comp.insert(
                {comp_idx, utils::ReformatHistogram(sum, _current_ds.data)});
          }
          // it's a explicit TFormula
            ...
            auto _tfunc = utils::ParseTFormula(
                iso.key(), iso.value()["TFormula"].get<std::string>(),
                _current_ds.data->GetXaxis()->GetXmin(),
                _current_ds.data->GetXaxis()->GetXmax());
            if (_tfunc.GetNdim() != _current_ds.data->GetDimension()) {
              throw std::runtime_error(
                  "the specified PDF TFormula has a different dimensionality "
                  "than the data");
            }
            auto thh = new TH1D(iso.key().c_str(), iso.key().c_str(),
            ...
            if (iso.value()["isotope"].is_string()) {
              ...
              _current_ds.comp.insert(
                  {comp_idx, utils::ReformatHistogram(comp, _current_ds.data)});
              ...
              throw std::runtime_error(
                  "unexpected entry " + iso.value()["isotope"].dump() +
                  "found in [\"fit\"][\"" + el.key() + "\"][\"" + elh.key() +
                  "\"][\"components\"][\"" + iso.key() + "\"][\"isotope\"]");

            if (!_current_ds.comp[comp_idx] or
                !_current_ds.comp_orig[comp_idx]) {
              throw std::runtime_error(
                  "invalid pointer found in component list at position " +
      ...
      if (!elh.value().contains("fit-range-x") and
          !elh.value().contains("fit-range")) {
        _vxr.push_back({_current_ds.data->GetXaxis()->GetBinCenter(1),
                        _current_ds.data->GetXaxis()->GetBinCenter(
                            _current_ds.data->GetNbinsX())});
      } else { // sanity check x-range
        auto x_range = elh.value().contains("fit-range-x")
                           ? elh.value()["fit-range-x"]
                           : elh.value()["fit-range"];

        if (!x_range.is_array())
          throw std::runtime_error(
              "wrong \"fit-range-x\" format, array expected");
        if (!(x_range[0].is_array() or x_range[0].is_number()))
          throw std::runtime_error("wrong \"fit-range-x\" format, array of "
                                   "arrays or numbers expected");
        ...
        if (x_range[0].is_array()) {
          ...
          for (auto &r : x_range) {
            if (!r.is_array())
              throw std::runtime_error(
                  "non-array member detected in \"fit-range-x\"");
            if (r.size() != 2)
              throw std::runtime_error(
                  "wrong number of entries in item of \"fit-range-x\"");
            if (!r[0].is_number() or !r[1].is_number()) {
              throw std::runtime_error(
                  "not-a-number value detected in \"fit-range-x\"");
            }
            _vxr.push_back({r[0], r[1]});
          }
        } else
          _vxr.push_back({x_range[0], x_range[1]});
      }

      // set fit range 1D
      if (_current_ds.data->GetDimension() == 1) {
        if (_vxr.empty())
          throw std::runtime_error("Something went wrong with the TH1 x-range");
        else {
          for (auto x : _vxr) {
            ...
            while (_x_ll <= 0)
              _x_ll++;
            while (_x_ul > _current_ds.data->GetNbinsX())
              _x_ul--;
            _current_ds.brange.push_back({_x_ll, _x_ul});
            BCLog::OutDetail(
                "Adding fit range x [" +
                std::to_string(_current_ds.brange.back().first) + ", " +
                std::to_string(_current_ds.brange.back().second) +
                "] (bins) [" +
                std::to_string(_current_ds.data->GetBinLowEdge(_x_ll)) + ", " +
                std::to_string(_current_ds.data->GetBinLowEdge(_x_ul) +
                               _current_ds.data->GetBinWidth(_x_ul)) +
                "] (x-units)");
          }
        }
      }
      // set fit range 2D
        ...
        if (!elh.value().contains("fit-range-y")) {
          _vyr.push_back({_current_ds.data->GetYaxis()->GetBinCenter(1),
                          _current_ds.data->GetYaxis()->GetBinCenter(
                              _current_ds.data->GetNbinsY())});
        } else { // sanity check y-range
          auto &y_range = elh.value()["fit-range-y"];
          if (!y_range.is_array())
            throw std::runtime_error(
                "wrong \"fit-range-y\" format, array expected");
          if (!(y_range[0].is_array() or y_range[0].is_number()))
            throw std::runtime_error("wrong \"fit-range-y\" format, array of "
                                     "arrays or numbers expected");
          ...
          if (y_range[0].is_array()) {
            ...
            for (auto &r : y_range) {
              if (!r.is_array())
                throw std::runtime_error(
                    "non-array member detected in \"fit-range-y\"");
              if (r.size() != 2)
                throw std::runtime_error(
                    "wrong number of entries in item of \"fit-range-y\"");
              if (!r[0].is_number() or !r[1].is_number()) {
                throw std::runtime_error(
                    "not-a-number value detected in \"fit-range-y\"");
              }
              _vyr.push_back({r[0], r[1]});
            }
          } else
            _vyr.push_back({y_range[0], y_range[1]});
        }
        ...
        if (_vxr.empty() or _vyr.empty())
          ...
          for (auto x : _vxr) {
            ...
            for (auto y : _vyr) {
              ...
              while (_y_ll <= 0)
                _y_ll++;
              while (_y_ul > _current_ds.data->GetNbinsY())
                _y_ul--;
              while (_y_ll <= _y_ul) {
                _current_ds.brange.push_back(
                    {_current_ds.data->FindBin(
                         _current_ds.data->GetXaxis()->GetBinCenter(_x_ll),
                         _current_ds.data->GetYaxis()->GetBinCenter(_y_ll)),
                     _current_ds.data->FindBin(
                         _current_ds.data->GetXaxis()->GetBinCenter(_x_ul),
                         _current_ds.data->GetYaxis()->GetBinCenter(_y_ul))});

                BCLog::OutDetail(
                    "Adding fit range TH2 global [" +
                    std::to_string(_current_ds.brange.back().first) + ", " +
                    std::to_string(_current_ds.brange.back().second) +
                    "] (bins) [" +
                    std::to_string(
                        _current_ds.data->GetXaxis()->GetBinLowEdge(_x_ll)) +
                    ", " +
                    std::to_string(
                        _current_ds.data->GetXaxis()->GetBinLowEdge(_x_ul) +
                        _current_ds.data->GetXaxis()->GetBinWidth(_x_ul)) +
                    " : " +
                    std::to_string(
                        _current_ds.data->GetYaxis()->GetBinLowEdge(_y_ll)) +
                    ", " +
                    std::to_string(
                        _current_ds.data->GetYaxis()->GetBinLowEdge(_y_ll) +
                        _y_bin_width) +
        ...
        throw std::runtime_error(
            "cannot set data ranges : TH3D is not supported yet");
      ...
      for (auto &r : _current_ds.brange) {
        if (r.first > _last)
          ...
          throw std::runtime_error(
              "illegal ranges in \"fit-range-x\" detected, did you specify "
  ...
  if (config.contains("observables")) {
    ...
    for (auto &el : config["observables"].items()) {
      ...
      if (!_tformula.IsValid() or _tformula.GetNpar() < 1 or
          _tformula.GetNdim() > 0) {
      ...
      for (int p = 0; p < _tformula.GetNpar(); ++p) {
        ...
        for (unsigned int idx = 0; idx < this->GetNParameters(); ++idx) {
          if (this->GetParameters().At(idx).GetName() == parname) {
            exists = true;
            // we do this here because we'd rather save BAT's internal parameter
            // index to access it later in HMixFit::CalculateObservables
      ...
      if (el.value().contains("multiply-fit-parameter-by-pdf-integral")) {
        BCLog::OutDebug("asked to scale observable '" + el.key() +
                        "' with PDF integral");
        // get range
        std::vector<std::pair<double, double>> _scale_range;
        if (el.value()["multiply-fit-parameter-by-pdf-integral"].contains(
                "range"))
          _scale_range = utils::CheckAndStoreRanges(
              el.value()["multiply-fit-parameter-by-pdf-integral"]["range"]);
        else {
          throw std::runtime_error("Need range to scale " + el.key() +
                                   " with pdf integral.");
        }
        // find dataset
        ...
        if (el.value()["multiply-fit-parameter-by-pdf-integral"].contains(
          ...
          std::string _dataset = utils::SafeROOTName(
              "_" +
              el.value()["multiply-fit-parameter-by-pdf-integral"]["dataset"]
                  .get<std::string>());
          ...
          if (_ds_number >= this->data.size()) {
            throw std::runtime_error("Corresponding dataset " + _dataset +
                                     " not found.");
          }
        } else {
          throw std::runtime_error("Integral range for observable " + el.key() +
                                   " needs association to dataset.");
        }
        ...
        for (int p = 0; p < _tformula.GetNpar(); ++p) {
          std::string parname =
              std::string("[") + _tformula.GetParName(p) + "]";
          int idx = std::stoi(_tformula.GetParName(p));
          BCLog::OutDebug("considering parameter " + parname +
                          ". Will compute integral of component nr. " +
                          std::to_string(idx) + " in data set nr. " +
                          std::to_string(_ds_number));
          if (!this->data[_ds_number].comp_orig[idx]) {
            throw std::runtime_error(
                "histogram for component nr. " + std::to_string(idx) +
                " in data set nr. " + std::to_string(_ds_number) +
                " is null. The problem might be in the definition of "
                "observable '" +
                el.key() + "'. Turn on debug mode for details.");
          }
          double integral = utils::IntegrateHistogram(
              this->data[_ds_number].comp_orig[idx], _scale_range);
          auto _pos = _expr_.find(parname);
          auto _len = parname.size();
          _expr_.replace(_pos, _len,
                         Form("(%.5e*%s)", integral, parname.c_str()));
        ...
        // FIXME: 'Error in <TFormula::SetParName>: Parameter p1 is not
        // defined.' will be thrown if there are "jumps" in the parameters
        // indices. e.g. is the TFormula is '[0]*x + [7]' then
        // TFormula::GetNpar() will return 8, but not all parameters 0..8 are
        // really defined. I don't know how to fix this.
        for (int p = 0; p < _tformula.GetNpar(); ++p) {
          std::string parname = _tformula.GetParName(p);
      ...
      BCLog::OutDetail(
          "adding observable '" + el.key() + "' (\"" +
          el.value().value("long-name", "") + "\" [" +
          el.value().value("units", "") + "]) with TFormula = \"" +
          _tformula.GetExpFormula().Data() + "\" in range = [" +
          std::to_string(el.value()["range"][0].get<double>()) + "," +
          std::to_string(el.value()["range"][1].get<double>()) + "]");

      // register observable in BAT
      this->AddObservable(el.key(), el.value()["range"][0].get<double>(),
                          el.value()["range"][1].get<double>(),
                          el.value().value("long-name", ""),
                          "(" + el.value().value("units", "") + ")");
  ...
  for (auto &it : data) {
    for (auto &r : it.brange) {
      ...
      for (int b = r.first; b <= r.second; ++b) {
        ...
        for (auto &h : it.comp) {
          pred +=
              it.livetime * parameters[h.first] * h.second->GetBinContent(b);
        }
...
void HMixFit::CalculateObservables(const std::vector<double> &parameters) {
  ...
  for (unsigned int i = 0; i < this->GetNObservables(); ++i) {
    ...
    for (int p = 0; p < _tf.GetNpar(); ++p) {
      // we saved the BAT internal parameter index in the TFormula parameter
      // name
      _tf.SetParameter(p,
                       parameters[std::stoi(std::string(_tf.GetParName(p)))]);
    }
...
TH1 *HMixFit::GetFitComponent(std::string filename, std::string objectname,
                              int &num_prim, TH1 *tf1_hist_format, double weight) {
  ...
  if (!obj)
    throw std::runtime_error(
        "HMixFit::GetFitComponent(): could not find object '" + objectname +
        "' in file " + filename);
  ...
  if (obj->InheritsFrom(TH1::Class())) {
    ...
    TObjString *_nprim_lgnd =
        dynamic_cast<TObjString *>(_tf.Get("number_of_primaries"));
    if (_nprim_lgnd) {
    ...
    auto _name_nodir =
        objectname.substr(objectname.find_last_of('/') + 1, std::string::npos);
    if (_name_nodir.substr(0, 3) == "M1_") {
      _nprim_gerda = dynamic_cast<TParameter<Long64_t> *>(
          _tf.Get("NumberOfPrimariesEdep"));
    } else if (_name_nodir.substr(0, 3) == "M2_") {
      _nprim_gerda = dynamic_cast<TParameter<Long64_t> *>(
          _tf.Get("NumberOfPrimariesCoin"));
    }
    ...
    if (_nprim == 1) {
      BCLog::OutWarning(
          "could not find suitable 'number of primaries' object in '" +
          filename + "', skipping normalization");
    } else
      th_orig->Scale(weight / _nprim);
    ...
    if (tf1_hist_format) {
      ...
      throw std::runtime_error("HMixFit::GetFitComponent(): a data format is "
                               "strictly needed if source is a TF1");
    }
    ...
    throw std::runtime_error("HMixFit::GetFitComponent(): object '" +
                             objectname + "' in file " + filename +
                             " isn't of type TH1 or TF1");
  }
...
void HMixFit::SetIntegrationProperties(json j) {
  ...
  this->SetIntegrationMethod(
      j.value("method", BCIntegrate::BCIntegrationMethod::kIntDefault));
  this->SetCubaIntegrationMethod(
      j.value("cuba-method", BCIntegrate::BCCubaMethod::kCubaDefault));
  ...
  if (j["integrator-settings"].is_object()) {
    ...
    if (jj[method].is_object()) {
      if (this->GetIntegrationMethod() !=
          BCIntegrate::BCIntegrationMethod::kIntCuba) {
        ...
        if (jj[method][cubamethod].is_object()) {
          if (jj[method][cubamethod]["niter-min"].is_number()) {
            this->SetNIterationsMin(
                jj[method][cubamethod]["niter-min"].get<long long>());
          }
          if (jj[method][cubamethod]["niter-max"].is_number()) {
            this->SetNIterationsMax(
                jj[method][cubamethod]["niter-max"].get<long long>());
          }
          if (jj[method][cubamethod]["rel-precision"].is_number()) {
            this->SetRelativePrecision(
                jj[method][cubamethod]["rel-precision"].get<double>());
          }
          if (jj[method][cubamethod]["abs-precision"].is_number()) {
            this->SetAbsolutePrecision(
                jj[method][cubamethod]["abs-precision"].get<double>());
          }
        }
        auto &jjj = jj[method][cubamethod];

        auto set_base_props = [&jjj](BCCubaOptions::General &m) {
          if (jjj["ncomp"].is_number())
            m.ncomp = jjj["ncomp"].get<int>();
          if (jjj["flags"].is_number())
            m.flags = jjj["flags"].get<int>();
        };
        ...
        case BCIntegrate::BCCubaMethod::kCubaVegas: {
          ...
          set_base_props(o);
          if (jjj["nstart"].is_number())
            o.nstart = jjj["nstart"].get<int>();
          if (jjj["nincrease"].is_number())
            o.nincrease = jjj["nincrease"].get<int>();
          if (jjj["nbatch"].is_number())
            o.nbatch = jjj["nbatch"].get<int>();
          if (jjj["gridno"].is_number())
            o.gridno = jjj["gridno"].get<int>();
        ...
        case BCIntegrate::BCCubaMethod::kCubaSuave: {
          ...
          set_base_props(o);
          if (jjj["nmin"].is_number())
            o.nmin = jjj["nmin"].get<int>();
          if (jjj["nnew"].is_number())
            o.nnew = jjj["nnev"].get<int>();
          if (jjj["flatness"].is_number())
            o.flatness = jjj["flatness"].get<double>();
          this->SetCubaOptions(o);
          break;
        }
        case BCIntegrate::BCCubaMethod::kCubaDivonne: {
          ...
          set_base_props(o);
          if (jjj["key1"].is_number())
            o.key1 = jjj["key3"].get<int>();
          if (jjj["key2"].is_number())
            o.key2 = jjj["key2"].get<int>();
          if (jjj["key3"].is_number())
            o.key3 = jjj["key1"].get<int>();
          if (jjj["maxpass"].is_number())
            o.maxpass = jjj["maxpass"].get<int>();
          if (jjj["border"].is_number())
            o.border = jjj["border"].get<double>();
          if (jjj["maxchisq"].is_number())
            o.maxchisq = jjj["maxchisq"].get<double>();
          if (jjj["mindeviation"].is_number())
            o.mindeviation = jjj["mindeviation"].get<double>();
          this->SetCubaOptions(o);
          break;
        }
        case BCIntegrate::BCCubaMethod::kCubaCuhre: {
          ...
          set_base_props(o);
          if (jjj["key"].is_number())
            o.key = jjj["key"].get<int>();
          this->SetCubaOptions(o);
          break;
        }
        case BCIntegrate::BCCubaMethod::kCubaDefault:
          break;
        case BCIntegrate::BCCubaMethod::NCubaMethods:
          break;
        }
      }
    }
...
void HMixFit::SaveHistogramsROOT(std::string filename) {
  ...
  for (auto &it : data) {
    ...
    for (auto &h : it.comp) {
      ...
      hcopy->Scale(it.livetime * this->GetBestFitParameters()[h.first]);
      // compute total model without changing the components
      if (!sum)
        sum = dynamic_cast<TH1 *>(hcopy->Clone());
      else
        sum->Add(hcopy);
    ...
    if (it.data->GetDimension() == 1) {
      ...
      TParameter<double> range_low("fit_range_lower",
                                   it.data->GetBinLowEdge(it.brange[0].first));
      TParameter<double> range_upp(
          "fit_range_upper", it.data->GetBinLowEdge(it.brange.back().second) +
    ...
    if (it.data->GetDimension() == 2) {
      ...
      // full x-range
      TParameter<double> range_low(
          "fit_range_lower_x", it.data->GetXaxis()->GetBinLowEdge(binx_low));
      TParameter<double> range_upp("fit_range_upper_x",
                                   it.data->GetXaxis()->GetBinLowEdge(binx_up) +
                                       bw_x);
      range_low.Write();
      range_upp.Write();
...
void HMixFit::SaveHistogramsCSV(std::string folder) {
  ...
  for (auto &it : data) {
    ...
    if (it.data->GetDimension() != 1) {
      BCLog::OutDebug(Form(
          "HMixFit::SaveHistogramsCSV(): %s is not a 1D histogram, skipping.",
          it.data->GetName()));
    }
    ...
    for (int b = 0; b <= it.data->GetNbinsX() + 1; ++b) {
      fout << it.data->GetBinCenter(b) << ", " << it.data->GetBinLowEdge(b)
      ...
      auto sig1 =
          utils::smallest_poisson_interval(0.682, sum->GetBinContent(b));
      auto sig2 =
          utils::smallest_poisson_interval(0.954, sum->GetBinContent(b));
      auto sig3 =
          utils::smallest_poisson_interval(0.997, sum->GetBinContent(b));
      ...
      fout << ", "
           << utils::normalized_poisson_residual(sum->GetBinContent(b),
                                                 it.data->GetBinContent(b));
    ...
    std::ofstream fout_orig(
        Form("%s/originals/%s.csv", folder.c_str(), it.data->GetName()));
    sum = nullptr;
    ...
    for (auto c : comps)
      fout_orig << ", " << c->GetName();
    fout_orig
        << ", 1sig_p, 1sig_m, 2sig_p, 2sig_m, 3sig_p, 3sig_m, norm_pois_res";
    fout_orig << '\n';
    ...
    for (int b = 1; b <= it.data_orig->GetNbinsX(); ++b) {
      ...
      fout_orig << ", " << sig1.second << ", " << sig1.first << ", "
                << sig2.second << ", " << sig2.first << ", " << sig3.second
                << ", " << sig3.first;

      fout_orig << ", "
                << utils::normalized_poisson_residual(
                       sum->GetBinContent(b), it.data_orig->GetBinContent(b));
      fout_orig << '\n';
    }
    for (auto c : comps)
      delete c;
...
void HMixFit::WriteResultsTree(std::string filename) {
  ...
  for (unsigned int p = 0; p < this->GetNVariables(); p++) {
    BCLog::OutDebug("Writing Parameter " + this->GetVariable(p).GetName() +
                    " to tree");
    bool isfixed =
        (p < this->GetNParameters() and this->GetParameter(p).Fixed());
    ...
    if (!isfixed) {
      ...
      glob_mode =
          p < this->GetNParameters() ? this->GetBestFitParameters()[p] : 0;
      glob_mode_error =
          p < this->GetNParameters() ? this->GetBestFitParameterErrors()[p] : 0;
    } else {
  ...
  for (auto ds : data) {
    ...
    double marg_scaling_factor, qt16_scaling_factor, qt84_scaling_factor,
        qt90_scaling_factor;

    TTree ttds(Form("counts_%s", ds.data->GetName()),
               "counts in selected regions for each parameter");
    ...
    ttds.Branch("fit_range_glob_mode_err", &bestErr_range,
                "fit_range_glob_mode_err/D");
    ...
    ttds.Branch("bi_range_glob_mode_err", &bestErr_bi,
                "bi_range_glob_mode_err/D");
    ...
    ttds.Branch("scaling_factor_glob_mode", &best_scaling_factor,
                "scaling_factor_glob_mode/D");
    ttds.Branch("scaling_factor_glob_mode_err", &bestErr_scaling_factor,
                "scaling_factor_glob_mode_err/D");
    ttds.Branch("scaling_factor_marg_mod", &marg_scaling_factor,
                "scaling_factor_marg_mod/D");
    ttds.Branch("scaling_factor_qt16", &qt16_scaling_factor,
                "scaling_factor_qt16/D");
    ttds.Branch("scaling_factor_qt84", &qt84_scaling_factor,
                "scaling_factor_qt84/D");
    ttds.Branch("scaling_factor_qt90", &qt90_scaling_factor,
                "scaling_factor_qt90/D");
    ...
    for (auto c : ds.comp_orig) {
      ...
      double best = isfixed ? this->GetParameter(c.first).GetFixedValue()
                            : this->GetBestFitParameters()[c.first];
      ...
      for (auto r : ds.brange) {
        ...
        auto bu_ = ch->FindBin(ds.comp[c.first]->GetBinLowEdge(r.second) +
                               ds.comp[c.first]->GetBinWidth(r.second));
        orig_range += ch->Integral(bl_, bu_);
      }
      best_range = orig_range * best;
      bestErr_range = orig_range * bestErr;
      marg_range =
          orig_range * (isfixed ? this->GetParameter(c.first).GetFixedValue()
                                : bch_marg.GetLocalMode());
      ...
      if (!ds.data_orig->InheritsFrom(TH2::Class())) {
        ...
        marg_bi =
            orig_bi * (isfixed ? this->GetParameter(c.first).GetFixedValue()
                               : bch_marg.GetLocalMode());
  ...
  for (auto &it : data) {
    ...
    for (auto &comp : it.comp) {
      ...
      auto msg = (i == it.comp.size() - 1 ? " └─ [" : " ├─ [") +
                 std::to_string(comp.first) + "] " +
                 std::string(comp.second->GetName()) + addr.str();
      BCLog::OutDebug(msg);
      addr.str("");
      addr.clear();
      i++;
...
double HMixFit::GetFastPValue(const std::vector<double> &parameters,
                              long niter) {

  BCLog::OutSummary("Computing fast (corrected) p-value with " +
                    std::to_string(niter) + " iterations");

  if (parameters.size() != this->GetParameters().Size()) {
    throw std::runtime_error(
        "GetFastPValue: input number of parameters does not match the number "
        "of defined model parameters");
  }
  ...
  for (auto &it : data) {
    ...
    for (auto &h : it.comp) {
      auto hcopy = dynamic_cast<TH1 *>(h.second->Clone());
      hcopy->Scale(it.livetime * parameters[h.first]);
      // compute total model
      if (!sum)
        sum = dynamic_cast<TH1 *>(hcopy->Clone());
      else
    ...
    if (nbins <= this->GetNFreeParameters()) {
      BCLog::OutSummary(Form(
          "cannot correct p-value for '%s' since nbins ('%i') < npars ('%i')",
          it.data->GetName(), nbins, this->GetNFreeParameters()));

    } else {
      double p = BCMath::CorrectPValue(p, this->GetNFreeParameters(), nbins);
    }
    BCLog::OutSummary(
        Form("p-value for data set '%s' => %g", it.data->GetName(), p));
  }
...
double HMixFit::Integrate(bool enable_offset) {
  ...
  if (GetBestFitParameters().size() != GetNParameters() &&
    ...
    BCLog::OutSummary("No best fit parameters available, needed for "
                      "integration. Please do the minimization first.");
    return 0.;
  }
  ...
  if (enable_offset) {
    ...
    BCLog::OutSummary(
        "LogIntegral (LogOffset " + std::to_string(_likelihood_offset) +
...
void HMixFit::PrintOptimizationSummary() {
  ...
  // └─┴┘
  BCLog::OutSummary(
      Form("Optimization summary for model \'%s\':", GetName().data()));
  BCLog::OutSummary(
      Form("  Number of parameters:  Npar = %i", GetNParameters()));
  BCLog::OutSummary("  Best fit parameters (global mode):");
  auto logl_best = this->LogLikelihood(this->GetBestFitParameters());
  BCLog::OutSummary("  LogLikelihood value at global mode: " +
                    std::to_string(logl_best));
  ...
  line = "    ┌";
  for (int i = 0; i < maxnamelength + 7; ++i)
    line += "─";
  line += "┬───────────────────┐";
  BCLog::OutSummary(line);
  BCLog::OutSummary(Form("    │ %-*s │ global mode       │", maxnamelength + 5,
                         "parameters"));
  line = "    ├";
  for (int i = 0; i < maxnamelength + 7; ++i)
    line += "─";
  ...
  for (size_t i = 0; i < best.size(); ++i) {
    ...
    line = Form("    │ [%2i] %-*s │ %7s ± %-7s │", int(i), maxnamelength,
                this->GetVariable(i).GetName().data(), val, err);
    if (this->GetParameter(i).Fixed())
      line += " (fixed)";
    BCLog::OutSummary(line);
  }
  line = "    └";
...
void HMixFit::PrintShortMarginalizationSummary() {
  ...
  // └─┴┘
  BCLog::OutSummary(
      Form("Marginalization summary for model \'%s\':", GetName().data()));
  BCLog::OutSummary(
      Form("  Number of parameters:  Npar = %i", GetNParameters()));
  ...
  line += "┬─────────────────────────────┐";
  BCLog::OutSummary(line);
  BCLog::OutSummary(Form("    │ %-*s │ marg. mode ± 1σ             │",
                         maxnamelength + 5, "parameters"));
  line = "    ├";
  for (int i = 0; i < maxnamelength + 7; ++i)
    line += "─";
  line += "┼─────────────────────────────┤";
  ...
  for (size_t i = 0; i < this->GetNVariables(); ++i) {
    bool isfixed = false;
    if (i < this->GetNParameters() and this->GetParameter(i).Fixed())
      isfixed = true;
    auto val = Form("%.*g", 2,
                    isfixed ? this->GetParameter(i).GetFixedValue()
                            : this->GetMarginalized(i).GetLocalMode());
    auto err1 = Form("%.*g", 2,
                     isfixed ? 0
                             : this->GetMarginalized(i).GetLocalMode() -
                                   this->GetMarginalized(i).GetQuantile(0.16));
    auto err2 = Form("%.*g", 2,
                     isfixed ? 0
                             : this->GetMarginalized(i).GetQuantile(0.84) -
                                   this->GetMarginalized(i).GetLocalMode());
    auto qt90 = Form("%.*g", 2,
                     isfixed ? 0 : this->GetMarginalized(i).GetQuantile(0.90));
    if (isfixed) {
      line = Form("    │ [%2i] %-*s │ %7s (fixed)             │", int(i),
                  maxnamelength, this->GetVariable(i).GetName().data(), val);
    } else if (this->GetMarginalized(i).GetLocalMode() >
                   this->GetMarginalized(i).GetQuantile(0.84) or
               this->GetMarginalized(i).GetQuantile(0.16) >
                   this->GetMarginalized(i).GetLocalMode()) {
      line = Form("    │ [%2i] %-*s │ < %7s (90%%C.L.)         │", int(i),
                  maxnamelength, this->GetVariable(i).GetName().data(), qt90);
    } else {
      line =
          Form("    │ [%2i] %-*s │ %7s + %-7s - %-7s │", int(i), maxnamelength,
               this->GetVariable(i).GetName().data(), val, err2, err1);
    }
    BCLog::OutSummary(line);
  }
  line = "    └";
